...
auto run(const crs_matrix_type& A, const Skema::Solver_Method::type solver,
  ...
  std::string history_filename =
      label + "_" + std::to_string(rank) + ".hist.json";
...
auto refine(const crs_matrix_type& A, matrix_type& U, vector_type& S,
            ...
            const size_t rank, Skema::AlgParams params) -> void {
  std::cout << "\n**************************************************"
            << std::endl;
  std::cout << "*********** Refining " << label << std::endl;
  std::cout << "**************************************************"
...
auto write_result(std::map<std::string, vector_type> dumpobj,
                  std::string& label, const size_t rank) {
  std::string fname;
  for (auto const& [key, val] : dumpobj) {
    fname = label + "_" + std::to_string(rank) + "." + key + ".txt";
...
int main(int argc, char* argv[]) {
  Kokkos::initialize(argc, argv);
  {
    ...
    size_t rank{1};
    rank = Skema::parse_int(args, "--rank", rank, 1, INT_MAX);
    if (rank == 1) {
      rank = RANK;
    }
    ...
    for (auto n = 0; n < NUM_REPS; ++n) {
      ...
      std::tie(u, s, v, r) =
          run(A, Skema::Solver_Method::type::ISVD, label, rank, isvd_params);
      dump["svals"] = s;
      dump["rnrms"] = r;
      ...
      std::tie(u, s, v, r) =
          run(A, Skema::Solver_Method::type::ISVD, label, rank, isvdopt_params);
      dump["svals"] = s;
      dump["rnrms"] = r;
      write_result(dump, label, rank);
      ...
      std::tie(u, s, v, r) = run(A, Skema::Solver_Method::type::SKETCHY_SVD,
                                 label, rank, sketchysvd_gauss_params);
      dump["svals"]        = s;
      dump["rnrms"]        = r;
      write_result(dump, label, rank);
      ...
      std::tie(u, s, v, r) = run(A, Skema::Solver_Method::type::SKETCHY_SPD,
                                 label, rank, sketchyspd_gauss_params);
      dump["svals"]        = s;
      dump["rnrms"]        = r;
      write_result(dump, label, rank);
      ...
      refine(A, u, s, r, label, rank, refine_sketchyspd_gauss_params);
      dump["svals"] = s;
      dump["rnrms"] = r;
      write_result(dump, label, rank);
      ...
      std::tie(u, s, v, r) = run(A, Skema::Solver_Method::type::SKETCHY_SVD,
                                 label, rank, sketchysvd_count_params);
      dump["svals"]        = s;
      dump["rnrms"]        = r;
      write_result(dump, label, rank);
      ...
      std::tie(u, s, v, r) = run(A, Skema::Solver_Method::type::SKETCHY_SPD,
                                 label, rank, sketchyspd_count_params);
      dump["svals"]        = s;
      dump["rnrms"]        = r;
      write_result(dump, label, rank);
      ...
      refine(A, u, s, r, label, rank, refine_sketchyspd_count_params);
      dump["svals"] = s;
      dump["rnrms"] = r;
      write_result(dump, label, rank);
