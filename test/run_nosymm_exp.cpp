...
int main(int argc, char* argv[]) {
  Kokkos::initialize(argc, argv);
  {
    ...
    double normalize = Skema::parse_real(args, "--normalize", 1.0,
    ...
    if (normalize != 1.0) {
      ...
      Kokkos::parallel_for(
          "normalize matrix", A.nnz(),
          KOKKOS_LAMBDA(const unsigned int i) { A.values(i) /= normalize; });
      Kokkos::fence();
      auto norm_end = std::chrono::system_clock::now();
      std::chrono::duration<double> norm_elapsed_seconds =
          norm_end - norm_start;
