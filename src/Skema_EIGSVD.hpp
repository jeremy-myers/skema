...
void eigs_monitorFun(void* basisEvals, int* basisSize, int* basisFlags,
...
auto save_primme_stats(const std::filesystem::path, const VectorT&,
                       const VectorT&, const PrimmeStats*);
...
extern "C" {
inline void eigs_monitorFun(void* basisEvals, int* basisSize, int* basisFlags,
                            int* iblock, int* blockSize, void* basisNorms,
                            int* numConverged, void* lockedEvals,
                            int* numLocked, int* lockedFlags, void* lockedNorms,
                            int* inner_its, void* LSRes, const char* msg,
                            double* time, primme_event* event,
      ...
      case primme_event_outer_iteration:
        ...
        for (int i = 0; i < *blockSize; ++i) {
          fprintf(primme->outputFile,
                  "OUT %ld blk %d MV %ld Sec %E tMV %E tORTH %E SV "
                  "%.16f "
                  "|r| %.16f\n",
      ...
      case primme_event_outer_iteration:
        ...
        for (int i = 0; i < *blockSize; ++i) {
          fprintf(primme_svds->outputFile,
                  "OUT %ld blk %d MV %ld Sec %E tMV %E tORTH %E SV %.16f "
                  "|r| %.16f\n",
...
template <typename MatrixType>
void primme_eigs(const MatrixType&, AlgParams);

template <typename MatrixType>
void primme_eigs(const MatrixType&, matrix_type&, vector_type&, vector_type&,
                 AlgParams);

template <typename MatrixType>
void primme_svds(const MatrixType&, AlgParams);

template <typename MatrixType>
void primme_svds(const MatrixType&, matrix_type&, vector_type&, matrix_type&,
