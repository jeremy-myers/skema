...
//       _row_idx(0) {
//   if (print_level_ > 0)
//     std::cout << "Initializing Sparse SketchySVD..." << std::flush;
...
//   std::cout << " "
//             << _Upsilon.stats.elapsed_time + _Omega.stats.elapsed_time +
//                    _Phi.stats.elapsed_time + _Psi.stats.elapsed_time
//             << " sec" << std::endl;
// }
...
//   crs_matrix_type A_init("A_init", static_cast<size_type>(wsize_), A.numCols(),
//                          A_init_nnz, A_init_values, A_init_row_map,
//                          A_init_entries);
...
//   dimredux_time = _Upsilon.stats.elapsed_time + _Omega.stats.elapsed_time +
//                   _Phi.stats.elapsed_time + _Psi.stats.elapsed_time;
//   total_time = dimredux_time;
//   std::cout << "i = " << std::right << std::setw(4) << iter
//             << ", Upsilon = " << _Upsilon.stats.time
//             << ", Omega = " << _Omega.stats.time
//             << ", Phi = " << _Phi.stats.time << ", Psi = " << _Psi.stats.time
//             << ", DimRedux total = " << std::right << std::setprecision(3)
//             << std::scientific << dimredux_time << ", update = " << std::right
//             << std::setprecision(3) << std::scientific << update_time
//             << ", total = " << std::right << std::setprecision(3)
//             << std::scientific << total_time << std::endl;
...
//     for (uint64_t ii = 0; ii < A.numCols(); ++ii) {
//       auto xtrow = xt0.row(ii);
//       for (uint64_t jj = 0; jj < xtrow.length; ++jj) {
//         Xt(ii, xtrow.colidx(jj)) = xtrow.value(jj);
//       }
//     }

//     for (uint64_t ii = 0; ii < A.numRows(); ++ii) {
//       auto yrow = y0.row(ii);
//       for (uint64_t jj = 0; jj < yrow.length; ++jj) {
//         Y(ii, yrow.colidx(jj)) = yrow.value(jj);
//       }
//     }

//     for (uint64_t ii = 0; ii < core_; ++ii) {
//       auto ztrow = zt0.row(ii);
//       for (uint64_t jj = 0; jj < ztrow.length; ++jj) {
...
//           A.graph.row_map(jj) - A.graph.row_map(idx.first);
//     uint64_t A_sub_nnz = A_sub_entries.extent(0);
//     crs_matrix_type A_sub("A_sub", static_cast<size_type>(wsize_), A.numCols(),
//                           A_sub_nnz, A_sub_values, A_sub_row_map,
//                           A_sub_entries);
...
//     Kokkos::parallel_for(
//         y.numRows(), KOKKOS_LAMBDA(const uint64_t ii) {
...
//     uint64_t Y_nnz = Y_values.extent(0);
...
//     total_time = update_time + dimredux_time;
...
//     // Add y to Y
//     size_t kk{0};
//     for (uint64_t ii = idx.first; ii < idx.second; ++ii) {
//       for (uint64_t jj = 0; jj < range_; ++jj) {
...
//   for (uint64_t irow = 0; irow < input_nrow; ++irow) {
//     for (uint64_t jcol = 0; jcol < input_ncol; ++jcol) {
...
//                       algParams.debug_level,
//                       algParams.outputfilename_prefix + "_omega");
//     std::cout << " " << _Omega.stats.elapsed_time << " sec" << std::endl;
...
//     A_init_row_map(jj - idx.first) =
//         A.graph.row_map(jj) - A.graph.row_map(idx.first);
//   uint64_t A_init_nnz = A_init_entries.extent(0);
...
//   total_time = _Omega.stats.elapsed_time;
//   std::cout << "i = " << std::right << std::setw(4) << iter
//             << ", Omega = " << _Omega.stats.time << ", update = " << std::right
//             << std::setprecision(3) << std::scientific << update_time
//             << ", total = " << std::right << std::setprecision(3)
//             << std::scientific << total_time << std::endl;
...
//     size_type kk{0};
//     for (uint64_t ii = idx.first; ii < idx.second; ++ii) {
//       for (uint64_t jj = 0; jj < _range; ++jj) {
...
//     update_time += timer.seconds();
//     total_time = update_time + _Omega.stats.elapsed_time;
...
//   Kokkos::fence();
//   if (_print_level > 0) {
//     std::cout << "  LMAP = " << _Omega.stats.time << " sec" << std::endl;
//   }
//   total_time += time;
...
//   Kokkos::fence();
//   matrix_type Ur("Ur", W.extent(1), min_mnw);
//   Kokkos::parallel_for(
...
//         auto ur2 = Kokkos::subview(Ur, Kokkos::ALL(), rr);
