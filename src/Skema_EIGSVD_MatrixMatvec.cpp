...
void svds_kernel_dense_matvec(void* x, PRIMME_INT* ldx, void* y,
                              PRIMME_INT* ldy, int* blockSize, int* transpose,
                              primme_svds_params* primme_svds, int* err) {
  ///! Capture exceptions here; don't propagate them to C code
  try {
    const EIGS_Kernel_Matrix kernel = *(EIGS_Kernel_Matrix*)primme_svds->matrix;
    const size_type nfeat           = kernel.nfeat;
    const size_type wsize           = kernel.wsize;
    range_type row_range;
    range_type xidx;
    ...
    for (auto irow = 0; irow < nrow; irow += wsize) {
      ...
      // Perform matvec
      // size_type xrow{*transpose == 0 ? ncol : nrow};
      // size_type yrow{*transpose == 0 ? row_range : ncol};

      if (*transpose == 0) {
        xidx = Kokkos::make_pair<size_t>(0, ncol);
        yidx = row_range;
      } else {
        xidx = row_range;
        yidx = Kokkos::make_pair<size_t>(0, nrow);
      }

      auto x_view = Kokkos::subview(x_view0, xidx, Kokkos::ALL());
      auto y_view = Kokkos::subview(y_view0, yidx, Kokkos::ALL());

      KokkosBlas::gemm(&transp, "N", 1.0, A_slice, x_view, 0.0, y_view);
    }
    *err = 0;
  } catch (const std::exception& e) {
    std::cout << "svds_kernel_dense_matvec encountered an exception: "
              << e.what() << std::endl;
    *err = 1;  ///! notify to primme that something went wrong
  }
}
